...
function M.setup(opts)
    ...
    local p = vim.deepcopy(require("yoruish.palette"))

    if opts and opts.on_colors then
        opts.on_colors(p)
    end

    require("yoruish.util").apply_accent(p, opts and opts.accent or 1)

    local groups = require("yoruish.groups").setup(p, opts)
